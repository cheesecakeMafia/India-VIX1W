...
class VIXCalculator:
    ...
    def organize_by_expiry(
        self, optionchain: pd.DataFrame, cutoff: int = 50
    ...
    def calculate_vix_variables(
        self, df: pd.DataFrame, time_to_expiry: float, atm_strike: float
    ) -> pd.DataFrame:
        ...
        for i, strike in enumerate(df["Strike"]):
            ...
            if strike < atm_strike:
                df.loc[i, "A"] = (
                    self.delta_k
                    * math.exp(self.risk_free_rate * time_to_expiry)
                    * (df.loc[i, "Put Bid"] + df.loc[i, "Put Ask"])
                ) / (2 * strike**2)
            elif strike > atm_strike:
                df.loc[i, "A"] = (
                    self.delta_k
                    * math.exp(self.risk_free_rate * time_to_expiry)
                    * (df.loc[i, "Call Bid"] + df.loc[i, "Call Ask"])
                ) / (2 * strike**2)
            else:  # ATM strike
                df.loc[i, "A"] = (
                    self.delta_k
    ...
    def calculate_sigma(
        self, df: pd.DataFrame, time_to_expiry: float, forward_price: float, atm_strike: float
    ) -> float:
